...
resource "azurerm_mysql_server" "mysql_server" {
  name                = "${local.resource_group_prefix}-${var.mysql_db_name}"
...
resource "azurerm_mysql_firewall_rule" "mysql_fw_rule" {
  ...
  resource_group_name = azurerm_resource_group.rg.name
  server_name         = azurerm_mysql_server.mysql_server.name
  start_ip_address    = azurerm_public_ip.Bastion_host_Public_IP.ip_address
  end_ip_address      = azurerm_public_ip.Bastion_host_Public_IP.ip_address
}
