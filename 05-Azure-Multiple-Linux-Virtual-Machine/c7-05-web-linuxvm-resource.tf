...
resource "azurerm_linux_virtual_machine" "web-linux-vm" {
  count = var.Virtual_machine_instance
  name = "${local.resource_group_prefix}-web1-linux-vm-${count.index}"
  computer_name = "web1-linux-vm-${count.index}"
  ...
  os_disk {
    name = "web1-linux-vm--${count.index}-${random_string.myrandom.id}"
    caching = "ReadWrite"
