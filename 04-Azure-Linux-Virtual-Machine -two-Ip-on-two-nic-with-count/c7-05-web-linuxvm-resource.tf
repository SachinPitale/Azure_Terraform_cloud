...
resource "azurerm_linux_virtual_machine" "web-linux-vm" {
  name = "${local.resource_group_prefix}-web1-linux-vm"
  computer_name = "web1-linux-vm"
  admin_username = "azureuser"
  size = "Standard_DS1_v2"
  resource_group_name = azurerm_resource_group.rg.name
  location = azurerm_resource_group.rg.location
  network_interface_ids = [ azurerm_network_interface.web_linuxvm_nic[0].id,  azurerm_network_interface.web_linuxvm_nic[1].id ]
  admin_ssh_key {
    username = "azureuser"
